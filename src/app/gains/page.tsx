...
export default function GainsPage() {
  ...
  const [mode, setMode] = useState<Mode>("biweekly");
  ...
  useEffect(() => {
    getAvailableWeeks(guildName, mode).then(setAvailablePeriods);
  }, [mode]);
  ...
  const uniqueLevels = Object.keys(groupedByLevels).map(Number).sort((a, b) => b - a);
  ...
  const getSelectedLabel = () => {
    if (selectedPeriod === "current") return mode === "biweekly" ? "Current Fortnight" : "Current Week";
    const period = availablePeriods.find(w => w.value === selectedPeriod);
    return period ? period.label : selectedPeriod;
  };

  const getTrophyIcon = (rank: number, size: "small" | "large" = "small") => {
    const sizeClass = size === "small" ? "h-8 w-8" : "h-12 w-12";
    switch (rank) {
      case 1: return <img src="/Golden_Goblet.gif" alt="Gold Trophy" className={`${sizeClass} object-contain`} />;
      case 2: return <img src="/Silver_Goblet.gif" alt="Silver Trophy" className={`${sizeClass} object-contain`} />;
      case 3: return <img src="/Bronze_Goblet.gif" alt="Bronze Trophy" className={`${sizeClass} object-contain`} />;
      default: return null;
    }
  };
  ...
  const copyToClipboard = () => {
    const header = mode === "biweekly" ? "Biweekly Experience Leaderboard" : "Weekly Experience Leaderboard";
    let text = `${header} — ${getSelectedLabel()}\n\n`;
  ...
  const isBiweekly = mode === "biweekly";
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-rose-50 via-white to-pink-50 relative overflow-hidden">
      ...
      <div className="relative z-20 px-4 sm:px-8 py-12">
        {/* Header */}
        <div className="max-w-7xl mx-auto mb-8" style={{ transform: `translateY(${mousePosition.y * 0.05}px)` }}>
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
            <div className="flex items-center gap-6">
              ...
              <div>
                <h1 className="text-4xl md:text-5xl font-serif font-black bg-gradient-to-r from-rose-600 via-rose-800 to-pink-600 bg-clip-text text-transparent mb-6 pb-2 leading-[1.1]">
                  {isBiweekly ? "Top Biweekly" : "Top Weekly"}
                </h1>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              {/* Mode toggle */}
              <div className="flex bg-white/80 backdrop-blur-sm border border-rose-100/50 rounded-2xl shadow-lg overflow-hidden">
                <button
                  type="button"
              ...
              <div className="relative">
                ...
                >
                  <option value="current">{isBiweekly ? "Current Fortnight" : "Current Week"}</option>
              ...
              >
                ...
                <span className="font-medium">{copySuccess ? "Copied!" : "Copy Results"}</span>
              </button>
            </div>
          </div>
        </div>
          ...
          <div className="flex items-center justify-center min-h-96">
            <div className="text-center">
              ...
              <p className="text-gray-500">
                {isBiweekly ? "Looks like it's been a quiet fortnight for leveling!" : "Looks like it's been a quiet week for leveling!"}
              </p>
            </div>
          </div>
        ) : (
              ...
              <div className="bg-white/60 backdrop-blur-sm rounded-2xl p-4 sm:p-6 shadow-lg border border-rose-100/50">
                <div className="text-center mb-4">
                  <h2 className="text-lg sm:text-xl font-bold bg-gradient-to-r from-yellow-600 to-yellow-800 bg-clip-text text-transparent mb-1">
                    🏆 Top 3 {isBiweekly ? "This Fortnight" : "This Week"}
                  </h2>
                </div>
                <div className="flex justify-center items-center gap-2 sm:gap-4">
                  {podium.map(({ rank, members, level }) => (
                    <div key={rank} className="flex flex-col items-center text-center">
                      <div className={`bg-white rounded-xl p-3 sm:p-4 shadow-md border hover:shadow-lg transition-all duration-300 ${
                        rank === 1 ? "border-yellow-300 border-2" : rank === 2 ? "border-gray-200" : "border-amber-200"
                      }`}>
                        <div className="mb-2 flex justify-center">{getTrophyIcon(rank)}</div>
                        <div className="text-xs sm:text-sm font-semibold text-gray-800 mb-2 min-h-[2rem] flex items-center">
                          {members.length === 1 ? members[0].name : `${members.length} tied`}
                        </div>
                        <div className="text-xs font-medium text-gray-600">+{level} levels</div>
                      </div>
                    </div>
                  ))}
                </div>
                  ...
                  return (
                    <div
                      ...
                      className={`rounded-2xl p-4 sm:p-6 border transition-all duration-200 hover:shadow-md ${
                        isPodium
                          ? rank === 1 ? "bg-gradient-to-r from-yellow-50 to-yellow-100 border-yellow-200"
                    ...
                    >
                      <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                        <div className="flex-shrink-0 flex items-center gap-3">
                          {isPodium && rank !== null && (
                            <div className="flex items-center">{getTrophyIcon(rank, "small")}</div>
                          )}
